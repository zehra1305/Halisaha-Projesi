...
exports.doluSaatleriGetir = async (req, res) => {
  try {
    ...
    // Bu tarihteki tüm randevuları bul
    const randevular = await Reservation.find({ tarih });
...
exports.randevuOlustur = async (req, res) => {
  try {
    ...
    const zatenVarMi = await Reservation.findOne({ tarih, saat });
    ...
    const randevularSon = await Reservation.find({ tarih });
