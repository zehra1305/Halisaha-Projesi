...
// --- KESİN DÜZELTİLMİŞ IMPORTLAR (screens klasörüne göre ayarlandı) ---

// Servis (screens'tan lib'e, oradan services'a in)
import '../services/api_service.dart'; 

// Modeller (screens'tan lib'e, oradan models'a in)
import '../models/customer.dart';
import '../models/message.dart';
import '../models/appointment.dart'; 
import '../models/duyuru.dart'; 

// Sayfalar (screens'ın hemen altındaki auth klasörüne in)
import 'auth/login_screen.dart'; // Çıkış için
...
class _AdminDashboardState extends State<AdminDashboard> {
  ...
  void initState() {
    super.initState();
    // IntL (Türkçe tarih formatı) başlatma
  ...
  // Randevuları API'den çeken fonksiyon (Mock şimdilik)
                    ...
                    child: Row(
                      children: [
                        Container(
                          ...
                          child: Center(
                            child: Text(customer.name[0].toUpperCase(), style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.green)),
                          ),
                        ),
                        const SizedBox(width: 15),
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(customer.email, style: const TextStyle(fontSize: 13, color: Colors.grey)),
                            ],
                          ),
                        ),
  ...
  // --- DUYURU YÖNETİM SAYFASI (GERÇEK VERİ) ---
      ...
      children: [
        ...
        Expanded(
          child: RefreshIndicator(
            onRefresh: _fetchDuyurular, // Yenileme fonksiyonu
            child: _isLoadingDuyurular
                ? const Center(child: CircularProgressIndicator())
                : _duyurularList.isEmpty
                    ? const Center(child: Text("Henüz duyuru eklenmemiş. Aşağı çekip yenileyin.", style: TextStyle(color: Colors.grey)))
                    : ListView.builder(
                              ...
                              children: [
                                ...
                                Expanded(
                                  child: Column(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      // Eğer duyuru modelinde baslik varsa burada gösterilebilir.
                                      // Text(duyuru.baslik, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                                      Text(
                                        duyuru.metin,
  ...
  void _sendDuyuru() async {
    if (_duyuruResimController.text.isEmpty || _duyuruMetinController.text.isEmpty) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Lütfen Resim URL ve Metin alanlarını doldurun!"), backgroundColor: Colors.red),
      );
      return;
    }
    ...
    bool success = await _apiService.addDuyuru(
      _duyuruBaslikController.text, // Başlık alanını kullanıyoruz
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Duyuru eklenirken hata oluştu! Sunucuyu kontrol edin."), backgroundColor: Colors.red),
      );
    }
  }
