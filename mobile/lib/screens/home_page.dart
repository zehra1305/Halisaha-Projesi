...
// ⚠️ DİKKAT: Bu dosyayı home_page.dart içinde kullanabilmek için,
// ProfileScreen'ın ayrı bir dosyada (profile_screen.dart) tanımlandığından emin olun
// ve doğru import yolunu (örneğin '../profile/profile_screen.dart') kullanın.
// Şu an için sadece ProfileScreen'ın var olduğunu varsayıyorum.
...
class _HomePageState extends State<HomePage> {
  ...
  // Alt menüdeki her bir ikonun gösterdiği widget'ı tanımlıyoruz.
  late final List<Widget> _widgetOptions;

  @override
  void initState() {
    super.initState();
    // Widget listesini initState içinde başlatıyoruz
    _widgetOptions = <Widget>[
      AnasayfaIcerigi(fetchDuyurular: fetchDuyurular), // 0: Anasayfa (Yaklaşan Maç, Duyurular)
      const Center(child: Text('İlanlar Sayfası')),   // 1: İlanlar
      const Center(child: Text('Randevu Ekle Sayfası')), // 2: Randevu Ekle
      const ProfileScreen(), // 3: Profil
    ];
  }

  // Duyuruları fetch eden fonksiyon, AnasayfaIcerigi'ne aktarılacak
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // Seçilen index'e ait widget'ı gösteriyoruz
...
// --- Anasayfa İçeriği Widget'ı (Eski Body) ---
// HomePage'in eski body kısmını (Slider, Maç, Duyurular) ayrı bir widget'a taşıyoruz.
class AnasayfaIcerigi extends StatelessWidget {
  final Future<List<Duyuru>> Function() fetchDuyurular;

  const AnasayfaIcerigi({required this.fetchDuyurular, super.key});
